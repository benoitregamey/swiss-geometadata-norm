...
class GeocatAPI():
    ...
    def get_metadata_group(self, uuid: str) -> str:
        """
        Fetches the group ID where the metadata belongs to.
        """
        ...
        while proxy_error:
            try:
                response = self.session.get(url=self.env + "/geonetwork/srv/fre/q", headers=headers, 
                            params=parameters)
        ...
        if int(xmlroot.find("./summary").attrib["count"]) == 0:
            print(f"{utils.warningred('The following Metadata does not exist : ') + uuid}")
            return None

        if xmlroot.find(".//dimension[@name='groupOwner']/category") is not None:
            if "value" in xmlroot.find(".//dimension[@name='groupOwner']/category").attrib:
                group_id = xmlroot.find(".//dimension[@name='groupOwner']/category").attrib["value"]
            else:
                print(f"{utils.warningred('Could not retrieve Group Id for : ') + uuid}")
                return None
        else:
            print(f"{utils.warningred('Could not retrieve Group Id for : ') + uuid}")
            return None
